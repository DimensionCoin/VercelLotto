...
import Stake from '../pages/Swap';
import BigSpender from '../pages/Lotto';
...
function EthHeader() {
  ...
  return (
    <header className='grid grid-cols-2 md:grid-cols-5 jusitfy-between items-center p-4 '>
        <div className='px-2 py-2 '> 
            <ConnectWallet  className='mt-1 mx-3'/>
        {isWrongNetwork ? (
            ""
        ) : (
            ''
        ...
        <div className='hidden md:flex md:flex-1 md:col-span-5 lg:flex lg:flex-1 lg:col-span-3 justify-center lg:space-x-4'>
            <div className=' flex bg-gradient-to-b from-gray-700 via-gray-900 to-black p-2 md:rounded-lg border-white border-2'>
                <button className='flex ml-auto text-right'>
                    <Link className='flex-row-2 font-bold text-lg hover:bg-gradient-to-r from-indigo-300 to-purple-400 text-white py-2 px-2 rounded font-bold' href={'./Matic'}>
                 Mattic Raffle
                 </Link>
                </button>
                <button className='flex ml-auto text-right'>
